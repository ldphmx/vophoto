...
for(i = 0; i < LJZname.length; i++) {
	if(!db.voice_images.findOne({"image_name": LJZname[i]})) {
		db.voice_images.save({"user_id": "b42c916c-3b1e-4235-85b7-451aea401218", "image_name": LJZname[i], "location": {"longitude": LJZLocation[0], "latitude": LJZLocation[1]}, "desc":"", "processed": true, "tags": ["lu-jia-zui","test"]});
...
for(i = 0; i < TAMname.length; i++) {
	if(!db.voice_images.findOne({"image_name": TAMname[i]})) {
		db.voice_images.save({"user_id": "b42c916c-3b1e-4235-85b7-451aea401218", "image_name": TAMname[i], "location": {"longitude": TAMLocation[0], "latitude": TAMLocation[1]}, "desc":"", "processed": true, "tags": ["tian-an-men","test"]});
...
for(i = 0; i < BMYname.length; i++) {
	if(!db.voice_images.findOne({image_name: BMYname[i]})) {
		db.voice_images.save({"user_id": "b42c916c-3b1e-4235-85b7-451aea401218", "image_name": BMYname[i], "location": {"longitude": BMYLocation[0], "latitude": BMYLocation[1]}, "desc":"", "processed": true, "tags": ["bing-ma-yong","test"]});
...
for(i = 0; i < SFname.length; i++) {
	if(!db.voice_images.findOne({image_name: SFname[i]})) {
		db.voice_images.save({"user_id": "b42c916c-3b1e-4235-85b7-451aea401218", "image_name": SFname[i], "time": SFdate, "desc":"", "processed": true, "tags": ["chun-jie","test"]});
...
for(i = 0; i < SMname.length; i++) {
	if(!db.voice_images.findOne({image_name: SMname[i]})) {
		db.voice_images.save({"user_id": "b42c916c-3b1e-4235-85b7-451aea401218", "image_name": SMname[i], "time": SMdate, "desc":"", "processed": true, "tags": ["xia-tian","test"]});
